...
void loadData(Crew crewsList, Flights ListFlights, Planes planesList);
void savedata(Crew crewsList, Flights ListFlights, Planes planesList);
...
int main() {
  ...
  while (choice != 0) {
    ...
    switch(choice) {
      ...
      case 21:
      {
        savedata(crewsList, ListFlights, planesList);
        break;
      }
      case 22:
      {
        loadData(crewsList, ListFlights, planesList);
        break;
      }
      default:
...
void savedata(Crew crewsList, Flights ListFlights, Planes planesList) {
  ...
  ListFlights.storeFlights();
  planesList.storePlanes();
}
...
void loadData(Crew crewsList, Flights ListFlights, Planes planesList) {
  ...
  cin >> i;
  crewsList.crewList.clear();
  for (int k = 0; k < i; ++k) {
    input >> name >> type >> status >> id;
    newCrew.setName(name);
    newCrew.setType(type);
    newCrew.setStatus(status);
    newCrew.setID(id);
    crewsList.crewList.push_back(newCrew);
  }
  ListFlights.loadFlights();
  planesList.loadPlanes();
}
