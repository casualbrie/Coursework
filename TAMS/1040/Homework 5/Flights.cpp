...
void Flights::storeFlights() {
  ...
  output << flightList.size() << endl;
  for (int i = 0; i < flightList.size(); ++i) {
    printEntry(flightList.at(i).getFlightID());
  }
  output.close();
}
...
void Flights::loadFlights() {
  ...
  for (int k = 0;k < i; ++k) {
    input >> planeID;
    input.ignore();
    getline(input, startPort);
    getline(input, endPort);
    input >> status;
    input.ignore();
    getline(input, startTime);
    getline(input, endTime);
    cin >> pilotID >> copilotID >> cabin1 >> cabin2 >> cabin3 >> numPass >> flightID;
    ...
    newFlight.setCabin1(cabin1);
    newFlight.setCabin2(cabin2);
    newFlight.setCabin3(cabin3);
    newFlight.setNumPass(numPass);
